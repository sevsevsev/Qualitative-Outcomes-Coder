...
//
// Vercel serverless function. Holds GEMINI_API_KEY server-side and calls
// Gemini through api/_lib/gemini.ts -- the browser never sees the key.
...
import { CODEBOOK_REGISTRY, CodebookType, getCodebook } from '../codebooks/index.js';
import { codeWithGemini } from './_lib/gemini.js';

// Model choice and the fallback now live in api/_lib/gemini.ts.
export { isModelUnavailableError } from './_lib/gemini.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // The public codebook explorer site is built from this same repo with
  // VITE_SITE=explorer. It has no coder screen, so this route is closed there
  // even if a GEMINI_API_KEY is set by mistake.
  ...
  try {
    const { parsed, modelUsed } = await codeWithGemini(apiKey, getCodebook(codebookType), items);
